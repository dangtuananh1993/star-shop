	  ...
	  if($product && taxonomy_exists($attribute)) {
		...
		foreach($terms as $term) {
		  if(in_array($term->slug, $options, true)) {
			...
			// echo "<br>";
			$attr_name = str_replace("pa_", "", $term->taxonomy);
			$value = get_field( $attr_name, 'term_' . $term->term_id );
			// print_r($value);
			echo "</pre>";
			$id = $name.'-'.$term->slug;
			$radios .= '<input  type="radio" id="'.esc_attr($id).'" name="'.esc_attr($name).'" value="'.esc_attr($term->slug).'" '.checked(sanitize_title($args['selected']), $term->slug, false).'><label style="background-color: '. $value .'" for="'.esc_attr($id).'" data-attr ="'. strtolower(esc_html(apply_filters('woocommerce_variation_option_name', $term->name))) .'" >'.esc_html(apply_filters('woocommerce_variation_option_name', $term->name)).'</label>';
		  }
		}
	  } else {
