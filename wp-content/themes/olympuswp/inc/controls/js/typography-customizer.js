( function( api ) {
	...
	api.controlConstructor['olympus-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.olympus-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the olympus_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'olympus_get_all_google_fonts_ajax',
								olympus_customize_nonce: olympus_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.olympus-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.olympus-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.olympus-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.olympus-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.olympus-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.olympus-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.olympus-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.olympus-font-variant' );
			...
			control.container.on( 'change', '.olympus-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.olympus-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.olympus-font-transform select',
				function() {
...
jQuery( document ).ready( function( $ ) {
	...
	$( '.olympus-font-family select' ).select2();

	$( '.olympus-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".olympus-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.olympus-font-variant' ).hide();
		}
	} );
